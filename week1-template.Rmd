...
map_nz1 <- map_nz +
  tm_shape(nz_elev) +
  tm_raster(alpha = 0.7) # version 4 you could use "col_alpha"
```
...
```{r include = TRUE}
map_nz1 + 
  tm_shape(nz_height) + # nz_height = preloaded data in spData with highest points in nz
  tm_dots()
```
...
```{r include=TRUE}
tm_shape(nz) +
  tm_polygons(col = "red", lwd = 3) #v4 uses fill = "red"
```
...
```{r include=TRUE}
map_nz +
  tm_compass(type = "4star", position = c("left", "top"), text.size = 1) +
  tm_scale_bar()
```
